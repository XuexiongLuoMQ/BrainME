...
def meta_train_maml(args, maml, lossfn, opt, tasks):
    ...
    for ep in range(args.epochs):
        ...
        opt.step()
        losses.append(meta_train_loss.item())
...
class PromptModel(nn.Module):
    def __init__(self, args):
        ...
        self.gib = MLP_subgraph(inp_dim=args.hid_dim, hid_dim=args.hid_dim*2, num_nodes=args.num_nodes+args.token_num, tau=args.gumbel_tau)
        #self.mse_criterion = torch.nn.MSELoss(reduction='mean')
        self.criterion = torch.nn.CrossEntropyLoss()
        self.prompt = HeavyPrompt(token_dim=args.token_dim, token_num=args.token_num, cross_prune=args.cross_prune, inner_prune=args.inner_prune)
    ...
    def prompt_gcn(self, x, adj, pg_x, pg_adj, cross_adj):
        """
        Args:
        ...
        pg_adj = pg_adj.unsqueeze(0).repeat(batch_size, 1, 1)
        pc_adj = torch.cat([pg_adj, cross_adj], dim=2)  #(B,P,P+N)
        cg_adj = torch.cat([cross_adj.transpose(1,2), adj], dim=2)  #(B,N,P+N)
        pg = torch.cat([pc_adj, cg_adj], dim=1)

        pg_x = pg_x.unsqueeze(0).repeat(batch_size, 1, 1) #(B,P,d)
        x = torch.cat([pg_x, x],dim=1) #(B,P+N,d)
        x = self.gcn(x, pg)
    ...
    def forward(self, data):
        ...
        out=[]
        x = self.fc_in(data.x)
        pg_x, pg_adj, cross_adj = self.prompt(x)
        x1, pg = self.prompt_gcn(x, data.adj_norm, pg_x, pg_adj, cross_adj)
        # x1, _ = self.prompt_gcn(x, data.adj_norm)
        g_h=torch.mean(x1, dim=1)
        out.append(g_h)

        #node_score = node_score_batch(x1) #(B,N)
        node_score = entropy_node_score(x1, pg) #(B,N)
        _, indx = torch.topk(node_score, k=self.k,largest = False) # (B, k) 
        sup_graph = self.sample_sub_graph(pg, indx, node_score)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--gumbel_tau', type=float, default=1.)
    ...
    if not args.meta_test:
        ...
        for name, param in maml.module.named_parameters():
            if name.startswith('prompt') or name.startswith('fc_in') or name.startswith("gib.linear2"):
